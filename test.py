...
# Add shop button properties
shop_btn_rect = pygame.Rect(window_size[0] // 2 - 100, 500, 200, 50)  # Shop button dimensions
...
fireball_speed = 10  # Speed of the fireball
...
def spawn_enemy():
    ...
    enemy_x = 1200
    enemy_y = 400
    enemy_speed = random.uniform(0.15, 0.7)  # Random speed
...
def draw_and_move_fireballs():
    ...
    for fireball in fireballs[:]:  # Iterate over a copy of the list
        ...
        if fireball["x"] > window_size[0]:  # Remove fireball if it goes off-screen
            ...
            pygame.draw.circle(screen, (255, 100, 0), (fireball["x"], fireball["y"]), fireball_radius)  # Draw the fireball
...
def draw_game():
    ...
    if is_shooting and time.time() - shoot_start_time < 0.1:  # Check if shooting state is active
        if shoot_character_img:
            screen.blit(shoot_character_img, (main_character_x, main_character_y))
    else:
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE and not is_jumping:  # Jump when space is pressed
                main_character_velocity_y = -6  # Reduced jump velocity for a slower and smoother jump
                is_jumping = True
    ...
    # Apply gravity and update main character position
    if is_jumping:
        main_character_velocity_y += 0.3  # Reduced gravity for a slower and smoother fall
        main_character_y += main_character_velocity_y
        if main_character_y >= 400:  # Reset position when character lands
            main_character_y = 400
            is_jumping = False
